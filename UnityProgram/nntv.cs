...
{
    public partial class nntv : UIPage
    {
        
		string postbbsapi, postimgapi, bbsisexistapi;
        private List<JsonInfo> listjson_nntv = new List<JsonInfo>();
        ...
        public void Run()
        {
            ...
            try
            {
                listjson_nntv = JsonConvert.DeserializeObject<List<JsonInfo>>(json);
                listjson_nntv.RemoveAll(t => t.Site != "livescore"); //移除非此爬虫网站的json
            }
            catch (Exception ex)
            {
        ...
        public List<string> isExist(string title)
        {
            ...
            foreach (var info in listjson_nntv)
            {
                try
                {
